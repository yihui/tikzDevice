...
\lang{R} environment by using either the \code{library()} or \code{require()}
functions like so:
...
<<tikzFormals,echo=F,results=hide>>=
cat(formatR::usage(tikz), file="tikzFormals.tex")
@

\tikzCodeInput[title={Usage},listing style=sweavechunk]{tikzFormals.tex}
...
\begin{figure}[!h]
\centering
\IfFileExists{figs/xelatexEx.pdf}{\includegraphics{figs/xelatexEx.pdf}}
